...
def _select_varlen_launch_config(
    query: torch.Tensor,
    *,
    ...
    if os.getenv("TRITON_QWEN3VL_HIGH_PARALLELISM", "0") != "1":
        max_q = int(max_length_q) if max_length_q is not None else None
        max_k = int(max_length_k) if max_length_k is not None else None
    ...
    max_q = int(max_length_q) if max_length_q is not None else int(query.shape[2])
    max_k = int(max_length_k) if max_length_k is not None else int(query.shape[2])

    # Smaller query tiles increase the number of launched programs for packed short sequences.
    if max_q <= 128:
        block_m = 32
    elif max_q <= 512 and head_parallelism <= 32:
        block_m = 32
    elif max_q <= 2048 and head_parallelism <= 32:
        block_m = 64
    else:
        block_m = 128

    block_n = 64 if max_k <= 1024 else 128
    if head_dim <= 64:
        num_warps = 4
    else:
        num_warps = 4 if block_m <= 64 else 8

    num_stages = 3 if max_k >= 512 else 2
...
def _varlen_flash_attention_forward(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    *,
...
class _VarlenFlashAttentionFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, cu_seq_lens_q, cu_seq_lens_k, sm_scale, max_length_q, max_length_k):
        ctx.sm_scale = float(sm_scale)
        ctx.save_for_backward(query, key, value, cu_seq_lens_q, cu_seq_lens_k)
        return _varlen_flash_attention_forward(
            query,
            key,
            value,
            cu_seq_lens_q=cu_seq_lens_q,
    ...
    def backward(ctx, grad_output):
        query, key, value, cu_seq_lens_q, cu_seq_lens_k = ctx.saved_tensors
        ...
        with torch.enable_grad():
            ...
            q_chunks = torch.split(q, q_lengths, dim=2)
            k_chunks = torch.split(k, k_lengths, dim=2)
            v_chunks = torch.split(v, k_lengths, dim=2)
            ref_output = torch.cat(
                [
                    dense_flash_attention_training(
                        q_chunk,
                        k_chunk,
                        v_chunk,
                        sm_scale=ctx.sm_scale,
            ...
            dq, dk, dv = torch.autograd.grad(ref_output, (q, k, v), grad_output)
...
def varlen_flash_attention(
    ...
    *,
    ...
    if torch.is_grad_enabled() and (query.requires_grad or key.requires_grad or value.requires_grad):
        return _VarlenFlashAttentionFunction.apply(
            query,
            key,
            value,
            cu_seq_lens_q,
