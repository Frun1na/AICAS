...
def _select_dense_launch_config(query: torch.Tensor) -> tuple[int, int, int, int]:
    ...
    # Favor more blocks when the workload has limited batch/head parallelism.
    if seq_len <= 256:
        block_m = 64
...
def dense_flash_attention(
    ...
    *,
    ...
    block_m, block_n, num_warps, num_stages = _select_dense_launch_config(query)
